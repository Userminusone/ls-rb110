...
The reason this code doesn't work is because a division is performed on two local variables containing integers on line 27, which means that the program performs integer division, which sometimes leads to unexpected results.

To fix this, we simply have to invoke the to_f method on one of the local variables to force ruby to calculate floating point division.
=end
# Term frequency - inverse document frequency:
# A measure of how important a term is to a document in a collection of documents
# (the importance increases proportionally to the term frequency in the document,
